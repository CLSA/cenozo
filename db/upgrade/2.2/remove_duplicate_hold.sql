DROP PROCEDURE IF EXISTS remove_duplicate_hold_now;
DELIMITER //
CREATE PROCEDURE remove_duplicate_hold_now()
  BEGIN
    SET @test = ( SELECT cenozo FROM application LIMIT 1 );
    IF @test != "2.2" THEN
      SELECT "Removing duplicate holds" AS "";
      CALL remove_duplicate_hold( NULL );
    END IF;
  END //
DELIMITER ;


SELECT "Creating new remove_duplicate_hold procedure" AS "";

DROP PROCEDURE IF EXISTS remove_duplicate_hold;
DELIMITER //
...
CALL remove_duplicate_hold_now();
DROP PROCEDURE IF EXISTS remove_duplicate_hold_now;
