...
-- Fri 12 Feb 2016 04:57:37 PM EST
...
USE `cenozo`;
DROP procedure IF EXISTS `cenozo`.`search`;
...
USE `cenozo`;
DROP procedure IF EXISTS `cenozo`.`run_search`;
...
BEGIN
  ...
  WHERE table_schema IN( DATABASE(), (
    ...
    AND table_name NOT IN ( 'service','unique_identifier_pool', 'variable_cache', 'writelog' )
  ...
  the_loop: LOOP
    ...
    SET @sql = CONCAT(
      ...
      "SELECT '", q,"', UTC_TIMESTAMP(), ",
			 ...
			 "IF( CHAR_LENGTH( ", column_name, " ) > 100, ",
                 "CONCAT( SUBSTRING( ", column_name, ", 1, 252 ), '...' ), ",
                 column_name, " ) ",
