...
-- Fri 13 Nov 2015 03:02:53 PM EST
...
USE `cenozo`$$
DROP TRIGGER IF EXISTS `cenozo`.`variable_cache_AFTER_INSERT` $$
USE `cenozo`$$
CREATE DEFINER = CURRENT_USER TRIGGER `cenozo`.`variable_cache_AFTER_INSERT` AFTER INSERT ON `variable_cache` FOR EACH ROW
BEGIN
  DELETE FROM variable_cache WHERE expiry <= UTC_TIMESTAMP();
END;$$


USE `cenozo`$$
DROP TRIGGER IF EXISTS `cenozo`.`variable_cache_AFTER_UPDATE` $$
USE `cenozo`$$
CREATE DEFINER = CURRENT_USER TRIGGER `cenozo`.`variable_cache_AFTER_UPDATE` AFTER UPDATE ON `variable_cache` FOR EACH ROW
BEGIN
  DELETE FROM variable_cache WHERE expiry <= UTC_TIMESTAMP();
END;$$
