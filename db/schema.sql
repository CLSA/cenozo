...
CREATE TABLE IF NOT EXISTS `cenozo`.`application` (
  ...
  `release_event_type_id` INT UNSIGNED NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `cenozo`.`script` (
  ...
  `pine_qnaire_id` INT UNSIGNED NULL DEFAULT NULL,
...
USE `cenozo`$$
DROP TRIGGER IF EXISTS `cenozo`.`script_BEFORE_INSERT` $$
USE `cenozo`$$
CREATE DEFINER = CURRENT_USER TRIGGER `cenozo`.`script_BEFORE_INSERT` BEFORE INSERT ON `script` FOR EACH ROW
BEGIN
...
USE `cenozo`$$
DROP TRIGGER IF EXISTS `cenozo`.`script_BEFORE_UPDATE` $$
USE `cenozo`$$
CREATE DEFINER = CURRENT_USER TRIGGER `cenozo`.`script_BEFORE_UPDATE` BEFORE UPDATE ON `script` FOR EACH ROW
BEGIN
  IF ( NEW.sid IS NULL AND NEW.pine_qnaire_id IS NULL ) or
     ( NEW.sid IS NOT NULL AND NEW.pine_qnaire_id IS NOT NULL ) THEN
    -- trigger column-not-null error
