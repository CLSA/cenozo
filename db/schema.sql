...
-- Mon 27 Apr 2015 03:11:59 PM EDT
...
CREATE TABLE IF NOT EXISTS `cenozo`.`jurisdiction` (
  ...
  INDEX `fk_site_id` (`site_id` ASC),
  INDEX `fk_postcode` (`postcode` ASC),
...
-- -----------------------------------------------------
-- Table `cenozo`.`person_first_address`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `cenozo`.`person_first_address` ;

CREATE TABLE IF NOT EXISTS `cenozo`.`person_first_address` (
  `person_id` INT UNSIGNED NOT NULL,
...
BEGIN
  ...
  REPLACE INTO participant_primary_address( person_id, address_id )
  SELECT proc_participant_id, address1.id
  FROM participant
  LEFT JOIN address AS address1 ON participant.person_id = t1.person_id
  WHERE participant.id = proc_participant_id
  AND address1.rank <=> (
    ...
    AND participant.person_id = address2.person_id
...
-- -----------------------------------------------------
-- procedure update_person_first_address
-- -----------------------------------------------------

USE `cenozo`;
DROP procedure IF EXISTS `cenozo`.`update_person_first_address`;
...
BEGIN
  ...
  SELECT proc_person_id, address1.id
  FROM person
  LEFT JOIN address AS address1 ON person.id = address.person_id
  WHERE person.id = proc_person_id
  AND address1.rank <=> (
    SELECT MIN( address2.rank )
    FROM address AS address2
    WHERE address2.active
    AND person.id = address2.person_id
