cenozoApp.defineModule({
  ...
  create: (module) => {
        ...
        return {
          ...
          controller: async function ($scope) {
            if (angular.isUndefined($scope.model))
              $scope.model = CnReportScheduleModelFactory.root;
            $scope.loading = true;
            ...
            $scope.model.addModel.heading =
              "Schedule " + response.data.title + " Report";
          },
        };
      },
    ]);
          ...
          controller: function ($scope) {
            ...
            $scope.model.viewModel.afterView(async function () {
              ...
              $scope.model.viewModel.heading =
                response.data.title + " Report Schedule";
            });
          },
        };
      },
          ...
          this.onNew = async function (record) {
            ...
            for (var column in this.parentModel.metadata.columnList) {
              ...
              if (angular.isDefined(meta.restriction_type)) {
                if (
                  "date" != meta.restriction_type &&
                  cenozo.isDatetimeType(meta.restriction_type)
                ) {
                  record[column] = null;
                } else if (
                  "boolean" == meta.restriction_type &&
                  meta.required
                ) {
                  record[column] = true;
                }
    ...
    cenozo.providers.factory("CnReportScheduleModelFactory", [
      "CnBaseModelFactory",
      "CnReportScheduleAddFactory",
      "CnReportScheduleListFactory",
      "CnReportScheduleViewFactory",
      "CnHttpFactory",
      function (
        CnBaseModelFactory,
        CnReportScheduleAddFactory,
        ...
        var object = function (root) {
          ...
          this.getMetadata = async function () {
            ...
            var [siteResponse, roleResponse, reportRestrictionResponse] =
              await Promise.all([
            ...
            await Promise.all(
              reportRestrictionResponse.data.map(async (restriction) => {
                var key = "restrict_" + restriction.name;
                ...
                if (dateType) {
                  ...
                  input.enumList
                    ...
                    .forEach((e) => {
                      ...
                      else if (e.value.match(/before/))
                        e.value = String(-parseInt(e.value));
                      else if (e.value.match(/after/))
                        e.value = String(parseInt(e.value));
                    });
                }
