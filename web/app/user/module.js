  ...
  create: (module) => {
    ...
    module.addInputGroup("", {
      active: {
        ...
        type: "boolean",
        help:
          "Inactive users will not be able to log in.  When activating a user their login failures count " +
    ...
    if (angular.isDefined(module.actions.edit)) {
      module.addExtraOperation("view", {
        ...
        operation: async function ($state, model) {
          await model.viewModel.resetPassword();
        },
      });
    }
    ...
    module.addExtraOperation("view", {
      ...
      classes: "btn-warning",
      isIncluded: function ($state, model) {
        return model.viewModel.listenToCallIncluded;
      },
      isDisabled: function ($state, model) {
        return model.viewModel.listenToCallDisabled;
      },
      operation: async function ($state, model) {
          ...
          this.onAddError = async function (response) {
            if (409 == response.status) {
              ...
              var response = await CnHttpFactory.instance({
                ...
                data: {
                  select: { column: ["name", "first_name", "last_name"] },
                },
              }).get();
          ...
          angular.extend(this, {
            ...
            resetPassword: async function () {
              var response = await CnModalConfirmFactory.instance({
                title: "Reset Password",
                message:
                  'Are you sure you wish to reset the password for user "' +
                  this.record.name +
                  '"',
              }).show();
                  ...
                  onError: async function (error) {
                    if (403 == error.status) {
                      await CnModalMessageFactory.instance({
                        ...
                        message:
                          'Sorry, you do not have access to resetting the password for user "' +
                          this.record.name +
                          '".',
                        error: true,
                      }).show();
          ...
          async function init(object) {
            ...
            if (angular.isDefined(object.languageModel))
              object.languageModel.listModel.heading =
                "Spoken Language List (if empty then all languages are spoken)";
          }
    ...
    cenozo.providers.factory("CnUserModelFactory", [
      ...
      function (
        ...
        var object = function (root) {
          ...
          this.showSiteList = function () {
            return CnSession.role.allSites;
          };

          // add additional details to some of the help text
          module.inputGroupList.findByProperty(
            "title",
            ""
          ).inputList.login_failures.help +=
            " Once it reaches " +
          ...
          this.getMetadata = async function () {
            ...
            var [roleResponse, siteResponse, languageResponse] =
              await Promise.all([
                CnHttpFactory.instance({
                  path:
                    "application_type/name=" +
                    CnSession.application.type +
                    "/role",
                  data: {
            ...
            this.metadata.columnList.role_id.enumList =
              roleResponse.data.reduce((list, item) => {
    ...
    cenozo.providers.factory("CnUserOverviewFactory", [
      ...
      function (
        ...
        if (CnSession.moduleList.includes("interview"))
          cenozo.insertPropertyAfter(columnList, "role", "assignment_uid", {
            title: "Assignment",
          });
