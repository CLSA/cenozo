...
try
{
  ...
  if( 'load' == $slot_action ) $session->set_arguments( $widget['args'] );
  ...
  $widget_class = util::get_full_class_name( 'ui\\widget\\'.$widget['name'] );
  
  // create the widget using the provided args then finish it
  $operation = new $widget_class( $widget['args'] );
  if( !is_subclass_of( $operation, 'cenozo\\ui\\widget' ) )
    throw new exception\runtime(
      'Invoked widget "'.$widget_class.'" is invalid.', 'WIDGET__SCRIPT' );
  if( 'load' == $slot_action ) $session->set_operation( $operation );
  ...
  if( 'load' == $slot_action ) $session->slot_push( $slot_name, $widget['name'], $widget['args'] );
}
