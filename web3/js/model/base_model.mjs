...
export class CN_base_model extends CN_base_object {
   ...
   * For documentation on the properties property look in the element/action/record.mjs file.
   */
  constructor(params) {
    super();
  ...
  /**
   * Returns a list of enum values needed by the property (null if no values are needed)
   * @param string prop_name: The name of the property to get enum values for
   * @param {type, enum} prop: An object with type and enum properties (as defined by the model)
   * return []
   */
  async get_enum_values(prop_name, prop) {
    if ("enum" == prop.type) {
      if (CN_common.is_object(prop.enum)) {
        // check if a get_enums function exists in the params
        if (CN_common.is_function(prop.enum.get_enums)) {
          return await prop.enum.get_enums(this);
        }

        // check if a path property exists in the params (it may be a string or a function)
        if (prop.enum.path) {
      ...
      return matches[1].split("','").map(v => ({ key: v, value: v, disabled: false }));
    } else if ("rank" == prop.type) {
      // populate the rank enum based on the max rank
      const response = await CN_api.get(this.get_base_path("api"), {
        select: { column: {
          column: `max(${this.get_name()}.rank)`,
          alias: "max_rank",
          table_prefix: false
        } },
      });
