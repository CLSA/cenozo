...
abstract class service extends \cenozo\base_object
{
  ...
  protected function finish()
  {
    ...
    if( 0 < $data_length )
    {
      ...
      if( false !== strpos( $this->mime_type, 'application/vnd' ) )
      {
  ...
  public function get_data()
  {
    if( is_null( $this->encoded_data ) )
    {
      ...
      if( 'text/csv' == $this->mime_type )
      {
        $this->encoded_data = $util_class_name::get_data_as_csv( $this->data );
      }
      else if( 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' == $this->mime_type ||
               'application/vnd.oasis.opendocument.spreadsheet' == $this->mime_type )
      {
        $report = lib::create( 'business\report' );
        $report->load_data( $this->data );
        $this->encoded_data = $report->get_file( $this->mime_type );
      }
