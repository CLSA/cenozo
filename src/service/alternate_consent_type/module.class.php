...
namespace cenozo\service\alternate_consent_type;
...
class module extends \cenozo\service\site_restricted_module
{
  ...
  public function prepare_read( $select, $modifier )
  {
    ...
    if( $select->has_column( 'accept_count' ) )
    {
      ...
      $join_sel->from( 'alternate_consent' );
      $join_sel->add_column( 'alternate_consent_type_id' );
      $join_sel->add_column( 'COUNT(*)', 'accept_count', false );

      $join_mod = lib::create( 'database\modifier' );
      $join_mod->join( 'alternate', 'alternate_consent.alternate_id', 'alternate.id' );
      $join_mod->where( 'accept', '=', true );
      $join_mod->group( 'alternate_consent_type_id' );

      // restrict to participants in this application
      if( $db_application->release_based )
      {
        $sub_mod = lib::create( 'database\modifier' );
        $sub_mod->where( 'alternate.participant_id', '=', 'application_has_participant.participant_id', false );
      ...
      $modifier->left_join(
        sprintf( '( %s %s ) AS alternate_consent_type_join_accept', $join_sel->get_sql(), $join_mod->get_sql() ),
        'alternate_consent_type.id',
        'alternate_consent_type_join_accept.alternate_consent_type_id' );
    ...
    if( $select->has_column( 'deny_count' ) )
    {
      ...
      $modifier->left_join(
        sprintf( '( %s %s ) AS alternate_consent_type_join_deny', $join_sel->get_sql(), $join_mod->get_sql() ),
        'alternate_consent_type.id',
