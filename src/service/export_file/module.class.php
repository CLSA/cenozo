...
class module extends \cenozo\service\module
{
  ...
  public function validate()
  {
    ...
    if( $this->service->may_continue() )
    {
      ...
      if( 'POST' == $method )
      {
        // make sure that the parent is an export
        if( 'export' != $this->get_parent_subject() )
        {
      ...
      else if( 'PATCH' == $this->get_method() )
      {
        ...
        // only the utility user can patch export files, and only to set the stage to started or failed
  ...
  public function pre_write( $record )
  {
    ...
    if( 'DELETE' == $method )
    {
      // make note of the export_file's filename to delete after the record has been removed
      $this->filename = $record->get_filename();
    }
    else if( 'POST' == $method )
    {
      // set the user and datetime
      $record->user_id = lib::create( 'business\session' )->get_user()->id;
  ...
  public function post_write( $record )
  {
    ...
    {
      // execute the export_file if the stage has been set to started and the progress to 1
      $file = $this->get_file_as_array();
      if( 2 == count( $file ) &&
          ...
          array_key_exists( 'progress', $file ) && 1 == $file['progress'] )
      {
        ...
        // generate the export_file's file
        $this->get_resource()->generate();
      }
    }
    else if( 'POST' == $method )
    {
      ...
      $curl = sprintf( 'curl -f -H %s -k %s',
        ...
        sprintf( "'%s/api/export_file/%d'", $db_application->url, $this->get_resource()->id )
      );
      $command = sprintf(
  ...
  /**
   * Stores a export_file's filename so it can be deleted after the record is removed
   * @var string $filename
   * @access private
   */
